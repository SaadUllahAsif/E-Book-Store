    ...
    <div class="wrapper">
      ...
      <div class="page-wrapper">
          ...
          <div class="content">					<div class="invoice-wrapper rounded border bg-white py-5 px-3 px-md-4 px-lg-5">
                ...
                <?php
                 $sample=$_GET ['orderno'];
                 $sql=" SELECT `customerid` FROM `orders` WHERE `id` = '$sample' ";
                 $result=mysqli_query($con,$sql);
                 if($result){
                  while($ok = $result->fetch_assoc()){
                 $key = $ok["customerid"];}
                  }else{
                    echo "Error From Order for Customer";
                  }
                $query="SELECT * FROM `customer` WHERE `id` = '$key'";
                $data=mysqli_query($con,$query);

    if($data){
      while($ok = $data->fetch_assoc()){

        $customeradderss = $ok ["address"];
        $customername = $ok ["firstname"];
        $customeremail = $ok ["email"];
        $customermethod = $ok ["method"];
        $customernumber = $ok ["mobilenumber"];
...
<address>
								...
								<br> Address: <?php echo $customeradderss; ?>
									<br> Email: <?php echo $customeremail; ?>
									<br> Phone: <?php echo $customernumber;?>
								</address>
							</div>
							...
							<div class="col-xl-3 col-lg-4">
								...
								<address>
									Payment Method:
									<span class="text-dark"><?php echo $customermethod; ?></span>
              ...
              <?php
               ...
               if($data)
               {
                 while($row = $data->fetch_assoc()){
                  ...
                  $bookprice=$row["price"];
                  $amount = ( 2 + $bookprice ); 
                  echo "<tr><td>" .$bookname. "</td><td> $ " .$bookprice. "</td></tr><br>";
							...
							<div class="col-lg-5 col-xl-4 col-xl-3 ml-sm-auto">
								<ul class="list-unstyled mt-4">
									<li class="mid pb-3 text-dark"> Subtotal
										<span class="d-inline-block float-right text-default">$ <?php echo  $bookprice; ?></span>
									</li>
									<li class="mid pb-3 text-dark">Shipping Cast
										<span class="d-inline-block float-right text-default">$ 2</span>
									</li>

									<li class="pb-3 text-dark">Total
										<span class="d-inline-block float-right">$<?php echo $amount; ?></span>
									</li>
								</ul>
